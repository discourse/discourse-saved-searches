  ...
  class ExecuteSavedSearches < ::Jobs::Base
    ...
    def execute(args)
      ...
      user.saved_searches.each do |saved_search|
        ...
        posts =
          results.posts.reject do |post|
            post.user_id == user.id || post.post_type != Post.types[:regular]
          end
        if posts.size > 0
          if saved_search.frequency != SavedSearch.frequencies[:immediately]
    ...
    def create_digest(user, saved_search, posts)
      posts_raw =
        if posts.size < 6
          ...
          posts
            .map do |post|
              I18n.t(
                "system_messages.saved_searches_notification.post_link_text",
      ...
      if tcf =
           TopicCustomField
             .joins(:topic)
             .where(name: custom_field_name, value: saved_search.query)
             .last
        PostCreator.create!(
          ...
          raw:
            I18n.t(
              "system_messages.saved_searches_notification.text_body_template",
              term: saved_search.query,
              posts: posts_raw,
            ),
