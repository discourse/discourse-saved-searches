...
describe Jobs::ExecuteSavedSearches do
  ...
  context "with saved searches" do
    ...
    it "send notification email" do
      ...
      expect { job.execute(user_id: user.id) }.to change { Notification.count }.by(1).and change {
    ...
    context "with recent post" do
      ...
      it "creates a notification if recent results are found" do
        expect { job.execute(user_id: user.id) }.to change { Notification.count }.by(1)
      end
      ...
      it "does not notify for small actions" do
        Fabricate(
          :post,
          topic: post.topic,
          raw: "Moved this to coupon category.",
    ...
    context "with own posts" do
      let!(:post) do
        Fabricate(:post, user: user, raw: "Check out these coupon codes for cool things.")
      end
    ...
    context "when not the first search" do
      ...
      before { job.execute(user_id: user.id) }

      it "does not notify about the same search results" do
        expect { job.execute(user_id: user.id) }.to_not change { Notification.count }
      end
      ...
      it "notifies about new results" do
        ...
        expect { job.execute(user_id: user.id) }.to change { Notification.count }.by(2)
      end
    end
    ...
    context "with daily digests" do
      ...
      it "creates a notification for each result" do
        ...
        expect { job.execute(user_id: user.id) }.to change { Topic.count }.by(2)
      end
      ...
      it "does not create a new topic if one exists" do
        ...
        expect { job.execute(user_id: user.id) }.to not_change { Topic.count }.and change {
                Post.count
              }.by(1)
      end
      ...
      it "does not create a new topic if not enough time passed" do
        ...
        expect { job.execute(user_id: user.id) }.to not_change { Topic.count }.and not_change {
                Post.count
              }
